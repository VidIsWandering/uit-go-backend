...
_Kế hoạch thực hiện Giai đoạn 2: (Kế hoạch chi tiết cho Module A được trình bày trong một tài liệu riêng được gửi kèm trong folder nộp báo cáo giai đoạn 1)_
...
### 3.11. ADR 013: Phân tách Security Group

- **Quyết định**: Áp dụng nguyên tắc Least Privilege cho Security Groups (Chaining SGs).
- **Lý do (Ưu tiên)**: Bảo mật chiều sâu (Defense in Depth).
- **Đánh đổi (Chấp nhận)**: Phức tạp quản lý cấu hình Terraform.

### 3.12. Các ADR cho Module A (Scalability)

- **ADR 001 (Async Processing)**: Chuyển sang Event-Driven với SQS để xử lý bất đồng bộ, tránh nghẽn cổ chai.
- **ADR 002 (Read Replicas)**: Sử dụng Read Replicas để scale thao tác Đọc.
- **ADR 003 (Caching Strategy)**: Dùng Redis làm Centralized Cache để giảm tải DB.
- **ADR 004 (Auto Scaling)**: Cấu hình Auto Scaling cho ECS (CPU-based) và RDS (Storage).
- **ADR 005 (Concurrency Control)**: Dùng Optimistic Locking để xử lý tranh chấp nhận chuyến.
