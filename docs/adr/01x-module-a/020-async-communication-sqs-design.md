...
const consumer = Consumer.create({
  queueUrl: 'https://sqs.ap-southeast-1.amazonaws.com/.../uit-go-driver-requests',
  handleMessage: async (message) => {
    ...
    await sqsClient.sendMessage({
      QueueUrl: 'https://sqs.ap-southeast-1.amazonaws.com/.../uit-go-driver-responses',
...
## Hướng Implement (Future - Giai đoạn 3)

### Week 1: Infrastructure
- [ ] Terraform: Create SQS queues (requests, responses, DLQs)
- [ ] IAM roles: Grant ECS tasks permission to publish/consume
- [ ] CloudWatch Alarms: Queue depth, DLQ messages

### Week 2: TripService Integration
- [ ] Add AWS SQS SDK dependency
- [ ] Implement event producer (publish FindDriverRequest)
- [ ] Implement event consumer (consume DriversFoundEvent)
- [ ] Add idempotency check (processedMessages table)

### Week 3: DriverService Integration
- [ ] Implement event consumer (poll driver-requests queue)
- [ ] Implement event producer (publish DriversFoundEvent)
- [ ] Add retry logic (3 attempts before DLQ)

### Week 4: Testing & Rollback Plan
- [ ] Load testing: 5000 RPS throughput
- [ ] Chaos engineering: DriverService down → messages queue up
- [ ] Rollback plan: Feature flag to switch back to REST
...
- **2024-11-20**: Đề xuất thiết kế trong Module A (design only, không implement)
- **Người đề xuất**: Nguyễn Quốc Bảo (Role B - Platform Engineer)
- **Decision**: Giữ REST cho Module A, consider SQS cho Giai đoạn 3
- **Reason**: Latency requirement (< 500ms) không compatible với SQS (+3s)
